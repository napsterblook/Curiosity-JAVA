...
public class ModelLoader {
	...
	public int appendVAO(float[] positions, float[] textureCoords) {
		int vao = createVAO();
		appendList(0, 2, positions);
		appendList(1, 2, textureCoords);
		unbind();
		return vao;
	}
	...
	public int loadPNG(String file) {
		TextureLoader image = TextureLoader.loadTexture(String.format("%s.png", file));
		int texture = image.getID();
		new ModelLoader().textures.add(texture);
		return texture;
	}
